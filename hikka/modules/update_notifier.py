...
class UpdateNotifierMod(loader.Module):
    """Shaxsiy userbotning so'nggi relizlarini kuzatib boradi va agar yangilanish kerak bo'lsa, sizni xabardor qiladi"""

    strings = {
        "name": "UpdateNotifier",
        "update_required": (
            "🥳 <b>Shaxsiy Yangilash mavjud!</b>\n\nUserbotning yangi versiyasi chiqdi\n🔮"
            " <b>Shaxsiy <s>{}</s> -> {}</b>\n\n{}"
        ),
        "more": "\n<i><b>🎥 Va {} ko'proq...</b></i>",
    }
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                doc=lambda: "Yangilanish xabarnomalarini o'chirib qo'ying",
    ...
    async def client_ready(self):
        try:
            ...
            raise loader.LoadError("Repo tashabbusi xatosi tufayli yuklay olmadi") from e

        self._markup = self.inline.generate_markup(
            [
                {"text": "🔄 Yangilash", "data": "hikka_update"},
                {"text": "🚫 E’tibor bermang", "data": "hikka_upd_ignore"},
            ]
        )
    ...
    async def update(self, call: InlineCall):
        """Jarayonni yangilash tugmachalarini bosish"""
        if call.data not in {"hikka_update", "hikka_upd_ignore"}:
            return

        if call.data == "hikka_upd_ignore":
            self.set("ignore_permanent", self.get_latest())
            await call.answer("Ushbu yangilanish haqida bildirishnomalar bekor qilindi")
            return
