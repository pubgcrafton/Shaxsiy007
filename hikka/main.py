...
class Hikka:
    ...
    async def _web_banner(self):
        """Shows web banner"""
        logging.info("✅ Veb sahifa rejimi konfiguratsiyaga tayyor")
    ...
    def _init_clients(self) -> bool:
        ...
        for session in self.sessions.copy():
            try:
                ...
                logging.error(
                    "Telefon raqami noto'g'ri.  Xalqaro formatdan foydalaning (+XX...) "
                    "va unda bo‘sh joy qo'ymang."
                )
                self.sessions.remove(session)
            except InteractiveAuthRequired:
                logging.error(
                    "%s seans tugatildi va qayta autentifikatsiya talab qilinadi",
                    session.filename,
                )
                self.sessions.remove(session)
        ...
        try:
          ...
          🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺🔺
            ...
            if not self.omit_log:
                print(logo1)
                web_url = (
                    f"🌐 Web sahifa: {self.web.url}\n"
                    if self.web and hasattr(self.web, "url")
                    else ""
                )
                logging.info(
                    "🔆 Shaxsiy userbot %s boshlandi\n🔏 GitHub majburiyati SHA: %s (%s)\n%s%s",
            ...
            logging.exception("Belgi xatosi")
    ...
    async def amain(self, first: bool, client: CustomTelegramClient):
        ...
        logging.debug("Jurnal konfiguratsiyasi yuklanmoqda...")
