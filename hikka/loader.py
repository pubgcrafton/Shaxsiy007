...
def replace_all_refs(replace_from: typing.Any, replace_to: typing.Any) -> typing.Any:
    """
    :summary: objga barcha havolalarni almashtirish uchun :mod:`gc` modulidan foydalanadi
               :attr:`replace_from` bilan :attr:`replace_to` (u eng yaxshisini qilishga harakat qiladi,
               nima bo'lganda ham).
     :param replace_from: Siz almashtirmoqchi bo'lgan ob'ekt.
     :param replace_to: Eski ob'ekt o'rniga kerakli yangi ob'ekt.
     :qaytaradi: replace_from
    """
    ...
    for referrer in _gc.get_referrers(replace_from):
        ...
        if isinstance(referrer, dict):
            ...
            logger.debug("%s qo‘llab-quvvatlanmaydi.", referrer)

    if hit is False:
        raise AttributeError(f"'{replace_from}' obyekti topilmadi")
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        ...
        if self._task:
            logger.debug("%s usuli uchun sikl toʻxtatildi", self.func)
        ...
        logger.debug("Loop ishlamayapti")
    ...
    def start(self, *args, **kwargs):
        ...
        if not self._task:
            logger.debug("%s usuli uchun sikl boshlandi", self.func)
            self._task = asyncio.ensure_future(self.actual_loop(*args, **kwargs))
        else:
            logger.debug("Ishlayotgan siklni boshlashga harakat qilindi")
    ...
    async def actual_loop(self, *args, **kwargs):
        ...
        while self.status:
            ...
            try:
                ...
                logger.exception("Loopni ishga tushirishda xatolik yuz berdi!")
...
class Modules:
    ...
    async def _request_join(
        ...
        if channel.id in self._db.get("hikka.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
                    f"Siz qo'shilishingiz kerak @{channel.username} ushbu moduldan foydalanish uchun"
                )

            return False

        if not isinstance(channel, Channel):
            raise TypeError("`tengdosh` maydon kanal bo'lishi kerak")
        ...
        _module.strings._base_strings["_hikka_internal_request_join"] = (
            f"💫 <b>Modul </b><code>{_module.__class__.__name__}</code><b> so'ragan"
            " kanalga qo'shiling <a"
            f" href='https://t.me/{channel.username}'>{utils.escape_html(channel.title)}</a></b>\n\n<b>❓"
            f" Sabab: </b><i>{utils.escape_html(reason)}</i>"
        )
        ...
        if assure_joined and not event.status:
            raise LoadError(
                f"Ushbu moduldan foydalanish uchun @{channel.username} ga qoʻshilishingiz kerak"
            )
    ...
    async def complete_registration(self, instance: Module):
        """Namunani to'liq ro'yxatdan o'tkazish"""
        with contextlib.suppress(AttributeError):
            _hikka_client_id_logging_tag = copy.copy(self.client.tg_id)
        ...
        *,
        ...
        if re.search(r"# ?scope: ?hikka_min", code):
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Shaxsiy version {'{}.{}.{}'.format(*ver)}+"
                    )
                )
