...
class InlineManager(
    ...
    def __init__(
        ...
        db: Database,
        allmodules: "Modullar",  # type: ignore
    ):
    ...
    async def _register_manager(
        ...
        try:
            ...
            logger.critical("Token muddati tugagan, bekor qilinmoqda...")
            return await self._dp_revoke_token(False)

        # Start the bot in case it can send you messages
        try:
            m = await self._client.send_message(self.bot_username, "/start shaxsiy tashabbus")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start shaxsiy tashabbus"
                )
            except Exception:
                logger.critical("Foydalanuvchilar botini blokdan chiqarib bo‘lmadi", exc_info=True)
                return False
        except Exception:
            self.init_complete = False
            logger.critical("Inline boshqaruvchi ishga tushirilmadi!", exc_info=True)
            return False
        ...
        async def new(*args, **kwargs):
            ...
            try:
                ...
                logger.critical("Ruxsatsiz olindi")
                await self._stop()
...
if __name__ == "__main__":
    raise Exception("Ushbu fayl modul sifatida chaqirilishi kerak")
