...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        # We create new bot
        logger.info("Foydalanuvchida bot yoʻq, yangisini yaratishga urinmoqda")
    ...
    async def _assert_token(
        ...
        logger.info("Bot tokeni JB da topilmadi, BotFather da qidirilmoqda")
    ...
    async def _dp_revoke_token(self, already_initialised: bool = True):
        if already_initialised:
            await self._stop()
            logger.error("Saylovda ziddiyat yuzaga keldi.  Tokenni bekor qilishga urinish...")
