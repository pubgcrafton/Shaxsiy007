    ...
    async def query_gallery(
        ...
        *,
        ...
        if not isinstance(force_me, bool):
            logger.error(
                "`force_me` uchun noto‘g‘ri tur.  Kutilayotgan `bool`, %s oldi",
                type(force_me),
            )
            return False
        ...
        if not isinstance(disable_security, bool):
            logger.error(
                "“Xavfsizlikni_o‘chirish” uchun noto‘g‘ri tur.  Kutilayotgan `bool`, %s oldi",
        ...
        if always_allow and not isinstance(always_allow, list):
            logger.error(
                "“Always_allow” uchun noto‘g‘ri tur.  Kutilayotgan `roʻyxat`, %s oldi",
                type(always_allow),
            )
            return False
        ...
        if (
            ...
            logger.error("So‘rovlar galereyasida yaroqsiz `elementlar` ko‘rsatilgan")
            return False
        ...
        for i in items:
            if "thumb_handler" not in i:
                ...
                if not isinstance(photo_url, str):
                    logger.error(
                        "`next_handler` dan yaroqsiz natija.  Kutilayotgan `str`, %s oldi",
                        type(photo_url),
                    )
                    continue
            else:
                ...
                if not isinstance(photo_url, str):
                    logger.error(
                        "`thumb_handler` dan noto‘g‘ri natija.  Kutilayotgan `str`, %s oldi",
                        type(photo_url),
                    )
                    continue
