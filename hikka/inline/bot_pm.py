...
class BotPM(InlineUnit):
    def set_fsm_state(
        ...
        if not isinstance(user, (str, int)):
            logger.error(
                "`set_fsm_state`dagi `foydalanuvchi` turi noto‘g‘ri.  Kutilayotgan `str` yoki `int`,"
                " %s oldi",
                type(user),
            )
            return False
        ...
        if not isinstance(state, (str, bool)):
            logger.error(
                "`set_fsm_state`dagi `holat` turi noto‘g‘ri.  Kutilayotgan `str` yoki `bool`,"
                " %s oldi",
                type(state),
            )
            return False
    ...
    def get_fsm_state(self, user: typing.Union[str, int]) -> typing.Union[bool, str]:
        if not isinstance(user, (str, int)):
            logger.error(
                "`get_fsm_state`dagi `foydalanuvchi` turi noto‘g‘ri.  Kutilayotgan `str` yoki `int`,"
                " %s oldi",
                type(user),
            )
            return False
